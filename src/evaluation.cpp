...
int Evaluator::heuristic(int piece, const Move& mv) {
  return [&]() -> int {
    switch (piece) {
      ...
      case 2:
        return man_values[flip_square(mv.move_vec.back())] - man_values[flip_square(mv.move_vec.front())];
      case 3:
        return king_values[flip_square(mv.move_vec.back())] - king_values[flip_square(mv.move_vec.front())];
      default:
        return 0;
    }
  }();
