...
void Game::play_game_at_depth(int depth) {
  ...
  do {
    ...
    if (debug_enabled)
      std::cout << "do move " << best_move.print_move()
                << " info: " << value * (main_board.get_side() ? 1 : -1) << " "
                << nodecout << " nodes @ "
                << static_cast<double>(nodecout) / ms_time << "k nodes/sec"
                << std::endl;
    else
  ...
  for (int i = 0; i < depth; ++i) {
    ...
    if (debug_enabled)
      std::cout << "\tinfo: "
                ...
                << " score: " << alg.score << " nodes: " << alg.movecount
                << std::endl;
  }
