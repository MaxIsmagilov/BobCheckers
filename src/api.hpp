...
void parse_moves(std::string &&input) {
  ...
  int next_space = 0;
  while (input[next_space] != '\0' && input[next_space] != ' ')
    next_space++;
...
int parse_input(std::string &&input) {
  ...
  // note: this command is separate from the PCI, it is used for terminal
  // gameplay
  ...
  // process `go depth` command
    ...
    value += 9;
    algo::Move_Info mi =
        algo::get_best_move(std::stoi(value), _MAIN_BOARD, _MAIN_TABLE);
...
void PCI_loop() {
  ...
  while (true) {
    ...
    if (std::cin.getline(buffer, 100)) {
      // process the command
      if (parse_input(std::string(buffer)))
        break;
    }
  }
}
