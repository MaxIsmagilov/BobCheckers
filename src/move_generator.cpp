...
void MoveGenerator::init_move_table() {
  ...
  for (int i = 0; i < 64; i++) {
    u64 pos   = 1ULL << i;
    u64 table = 0ULL;
    if (pos & ~utils::A_FILE & ~utils::RANK_8) table |= 1ULL << (i - 9);
    if (pos & ~utils::H_FILE & ~utils::RANK_8) table |= 1ULL << (i - 7);
    temp_table[0][i] = table;
  }
  for (int i = 0; i < 64; i++) {
    u64 pos   = 1ULL << i;
    u64 table = 0ULL;
    if (pos & ~utils::A_FILE & ~utils::RANK_1) table |= 1ULL << (i + 7);
    if (pos & ~utils::H_FILE & ~utils::RANK_1) table |= 1ULL << (i + 9);
  ...
  for (int j = 0; j < 3; j++) {
    for (int i = 0; i < 32; i++) {
      ...
      for (int k = 0; k < 32; k++) {
        if (temp_table[j][utils::indexto64(i)] & (1ULL << utils::indexto64(k))) {
  ...
  for (int i = 0; i < 64; i++) {
    u64 pos   = 1ULL << i;
    u64 table = 0ULL;
    if (pos & ~(utils::A_FILE | utils::B_FILE | utils::RANK_8 | utils::RANK_7)) table |= 1ULL << (i - 18);
    if (pos & ~(utils::H_FILE | utils::G_FILE | utils::RANK_8 | utils::RANK_7)) table |= 1ULL << (i - 14);
    temp_table[0][i] = table;
  }
  for (int i = 0; i < 64; i++) {
    u64 pos   = 1ULL << i;
    u64 table = 0ULL;
    if (pos & ~(utils::A_FILE | utils::B_FILE | utils::RANK_2 | utils::RANK_1)) table |= 1ULL << (i + 14);
    if (pos & ~(utils::H_FILE | utils::G_FILE | utils::RANK_2 | utils::RANK_1)) table |= 1ULL << (i + 18);
    temp_table[1][i] = table;
  }
  for (int i = 0; i < 64; i++) {
    temp_table[2][i] = temp_table[0][i] | temp_table[1][i];
...
std::vector<Move> MoveGenerator::get_captures(const int square) {
  ...
  while (bb) {
    ...
    while (att) {
      ...
      if (((1UL << end) & ~all_occ) &&
          ((1UL << utils::indexto32((utils::indexto64(start) + utils::indexto64(end)) / 2)) & defending_occ)) {
        ...
        if (temp.size() == 0 || end / 4 == 0 || end / 4 == 7) moves.emplace_back(start, end);
  ...
  while (bb) {
    ...
    while (att) {
      ...
      if (((1UL << end) & ~all_occ) &&
        ...
        // if there are no future captures, continue
        if (temp.size() == 0) moves.emplace_back(start, end);
...
std::vector<Move> MoveGenerator::get_all_captures() {
  ...
  for (int i = 0; i < 32; ++i) {
    ...
    const int piece = [&]() -> int {
      if (board[0] & (1UL << i)) return 0;
      if (board[1] & (1UL << i)) return 1;
      if (board[2] & (1UL << i)) return 2;
      if (board[3] & (1UL << i)) return 3;
      return 0;
    }();
    ...
    for (Move mw : newmoves) {
      ...
      moves.back()._hvalue = Evaluator::heuristic(piece, moves.back()) * moves.back().move_vec.size();
    }
  }
...
std::vector<Move> MoveGenerator::operator()() {
  ...
  // swap the two moves at the end of the list with the best moves (keep the
  // first one at the beginning)
  if (moves.size() > 2) {
    std::swap(moves[1], moves.back());
  }
  if (moves.size() > 3) {
    std::swap(moves[2], moves[moves.size() - 2]);
  }
