...
Algorithm::Algorithm(const Board& bd, TranspositionTable& tt) : table(tt), board_stack(bd) {}
...
Move Algorithm::search(int depth) {
  ...
  for (const Move& mw : moveslist) {
    ...
    int new_value = -negascout(depth - 1, -beta, -alpha, false);
    if (new_value > value) {
      value     = new_value;
      best_move = mw;
    }
...
int Algorithm::negascout(int depth, int alpha, int beta, const bool cut) {
  Board&            bd = board_stack.get_top();
  MoveGenerator     mg(bd);
  std::vector<Move> moveslist = mg();

  const int LMR_R = (depth > 6) ? 2 : depth / 3;
  ...
  if (entry._type != TTUtils::FAIL) {
    if (entry._type == TTUtils::EXACT) return entry._value;
    if (entry._type == TTUtils::LBOUND) alpha = std::max(alpha, entry._value);
    if (entry._type == TTUtils::UBOUND) beta = std::min(beta, entry._value);
    if (alpha >= beta) return entry._value;
  }
  ...
  if (moveslist.size() == 0) return -(game_over + depth);
  ...
  // negascout through the move list
  for (u32 i = 0; i < moveslist.size(); ++i) {
    board_stack.make_move(moveslist[i]);

    value = std::max(value, -negascout(depth - 1, -beta, -alpha, can_cut));
    if (alpha < value && value < original_beta && i > 0) {
      value = std::max(value, -negascout(depth - 1, -original_beta, -value, can_cut));
    }
...
int Algorithm::quiescence(int depth, int alpha, int beta) {
  ...
  if (depth == 0 || !mg.captures_available()) return Evaluator::eval(bd);
