...
TranspositionTable::TranspositionTable() noexcept { std::fill(table.begin(), table.end(), failed_entry); }

TTUtils::TT_Entry TranspositionTable::find(const u64 key) {
  return (key == table[get_index(key)]._key) ? table[get_index(key)] : failed_entry;
}
